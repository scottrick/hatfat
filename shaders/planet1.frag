...
{	
	...
	if (value > 0.9)
	{
		float finalValue = value - 0.1;
		finalValue = finalValue * finalValue;
		gl_FragColor = vec4(finalValue, finalValue, finalValue, 1.0);
	}
	else if (value > 0.6)
	{
		float fMod = value * 0.2;
		//gl_FragColor = vec4(0.43 + fMod, 0.16 + fMod, 0.06 + fMod, 1.0);
		gl_FragColor = vec4(0.43 * value, 0.4 - fMod, 0.06 * value, 1.0);
	}
	else if (value > 0.0)
	{		
		float fMod = value * 0.1;
		gl_FragColor = vec4(0.43 * value, 0.2 + fMod, 0.06 * value, 1.0);
	}
	else if (value > -0.05)
	{
		gl_FragColor = vec4(0.0, 0.0, 1.0 + 10.0 * value, 1.0);
	}
	else
	{
		gl_FragColor = vec4(0.0, 0.0, 0.5 + 0.25 * value, 1.0);
	}
}
